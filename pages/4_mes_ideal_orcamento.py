...
# ==============================================
# AGRUPAR POR DESTINO (COM TEMP)
# ==============================================
agg = (
    df_filtro.groupby("DESTINO", as_index=False)
    .agg({
        "TARIFA": "mean",
        "TEMP_MEDIA": "mean"
    })
...
agg["TARIFA_MEDIA"] = agg["TARIFA"].round(0)
agg["TEMP_MEDIA"] = agg["TEMP_MEDIA"].round(1)
...
agg["CLIMA"] = agg["TEMP_MEDIA"].apply(clima_label)
...
mais_frio = agg.loc[agg["TEMP_MEDIA"].idxmin()]
mais_quente = agg.loc[agg["TEMP_MEDIA"].idxmax()]
...
<b>• Destino mais frio no mês:</b> {mais_frio['DESTINO']} — {mais_frio['TEMP_MEDIA']:.1f}°C ({mais_frio['CLIMA']})<br><br>
<b>• Destino mais quente no mês:</b> {mais_quente['DESTINO']} — {mais_quente['TEMP_MEDIA']:.1f}°C ({mais_quente['CLIMA']})<br><br>
<b>• Diferença de preços entre o mais caro e barato:</b> R$ {pior['TARIFA_MEDIA'] - melhor['TARIFA_MEDIA']:.0f}
</div>
""", unsafe_allow_html=True)
